...
if (length(args) != 2) {
  stop(
    "Usage : Rscript plot_trophic_abundance.R INPUT_PHYLOSEQ_DIRECTORY OUTPUT_PHYLOSEQ_DIRECTORY"
  )
}
...
if (!dir.exists(INPUT_PS)) {
  stop("[R] : Input PS folder doesn't exist")
}


if (!dir.exists(OUTPUT_PS)) {
  dir.create(OUTPUT_PS, recursive = TRUE)
}
# ============== Libraries ============ #
library(taxinfo)
library(MiscMetabar)
    ...
    for (cluster in clusters) {
      ...
      OTU_table <- read.csv(
        paste0(
          ...
          "_",
          cluster,
          "_FungalTraits_enhanced_FunGuild/otu_table.csv"
        ),
        header = TRUE,
        row.names = 1,
        sep = "\t"
      ...
      TAX_table <- read.csv(
        paste0(
          ...
          "_",
          cluster,
          "_FungalTraits_enhanced_FunGuild/tax_table.csv"
        ),
        header = TRUE,
        row.names = 1,
        sep = "\t"
